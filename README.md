...
export PYTHONPATH=$PYTHONPATH:Your_CodePath/TractSegWithLabelEmbedding
```
* You can `directly test` the network using the provided data in the `example` fold and network model (download [link1]( https://pan.baidu.com/s/1EjvdWTomN6D3inC7wrSpaw?pwd=5gv3)-> password:5gv3; [link2]()) after modifying the corresponding paths in 'Your_CodePath/TractSegWithLabelEmbedding/bin/`ExpRunner_test.py`':
```
